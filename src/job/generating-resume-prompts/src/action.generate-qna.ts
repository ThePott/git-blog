...
const makePrompt = (fullName: string): string => {
    ...
    const jobName = fullName
        ...
        .join("__")
    const resumesDir = path.resolve(__dirname, "generated.qna")
    return `
...
- ${__dirname}/targets.qna/${fullName}.md 의 질문에 답해야 해.
...
1. 없는 사실을 만들지 마
2. ${__dirname}/resources/* 에 있는 내용 중, 답변에 들어갈 필요가 없다고 판단하면 제외해
3. ${__dirname}/resources/* 에 보완되어야 할 내용이 있으면 물어봐.
`
}

const generateQnaPrompts = () => {
    ...
    const qnaPromptsDir = path.resolve(__dirname, "generated.qna.prompts")

    fullNameArray.forEach((fullName) => {
        const targetPath = path.join(qnaPromptsDir, `${fullName}.md`)
    ...
    console.log("created qna prompts")
}

export default generateQnaPrompts
