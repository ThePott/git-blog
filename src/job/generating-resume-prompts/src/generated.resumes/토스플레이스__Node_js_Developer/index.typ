...
#grid(
    ...
    [
        = 하흥주
        #v(rem(0.5))
        // [수정] 타이틀 변경: JD 핵심 키워드 반영
        #additional_title("수작업을 시스템으로 자동화하는 Node.js 개발자")
        #v(rem(2))
...
// [수정] 자기소개: 업무 자동화, 반복 문제 패턴화 경험 강조
*반복되는 수작업을 시스템으로 자동화합니다*

입시 학원에서 강사로 일하며 비효율적인 수작업을 발견했습니다. 진도표와 오답 과제 현황을 수기로 동기화하는 데 학생당 5분, 수업당 50~100분이 소요되었습니다. 이를 해결하고자 파이썬과 구글 시트로 진도 및 오답 관리 시스템을 만들어 반복 작업을 자동화했습니다.

이후 CLI의 한계를 극복하고자 웹앱으로 발전시켰고, 서버 응답 속도 22배 향상(7초 → 0.3초)을 달성했습니다. 또한 개발 환경에서 반복되는 Tmux 세션 관리 문제를 패턴화하여 CLI 도구(Harpooning)로 만들어 세션 등록 시간을 31배 단축했습니다.

반복적인 문제를 빠르게 패턴화하고, 재사용 가능한 모듈로 만드는 개발자가 되고자 합니다.

== 기술 스택

// [수정] 기술 스택 순서: Node.js 중심으로 재배치
Node.js, TypeScript, Express, React, PostgreSQL, Prisma, Git, REST API
...
// [수정] Harpooning 먼저 배치: 반복 작업 자동화 CLI 도구
=== Harpooning Tmux 세션 관리자
#dim_text("개인/기타\n2026. 04. ~ 2026. 04.")

#additional_title("반복되는 세션 관리 작업을 CLI로 자동화")
...
#additional_title("수기 동기화 → 웹앱 자동화 시스템")
