...
#grid(
    ...
    [
        = 하흥주
        #v(rem(0.5))
        #additional_title("렌더링 성능을 최적화하는 프론트엔드 개발자") // [수정] JD 핵심: 브라우저 렌더링 성능 최적화
        #v(rem(2))
...
== 자기소개
// [수정] 성능 최적화, 대량 데이터 처리 경험 강조

*대량 데이터 렌더링 성능을 최적화합니다*

학원 관리 시스템을 웹앱으로 제작하며 수백 개의 셀을 가진 그리드 테이블에서 심각한 렌더링 지연을 경험했습니다. 문제의
본질은 불필요한 DOM 노드 생성이었고, TanStack Virtual을 도입해 가상 스크롤로 뷰포트 내 노드만 렌더링하여 입력 반응
시간을 20.38ms에서 2.28ms로 89% 단축했습니다.

또한 서버 응답 속도 7초 → 0.3초(22배 향상)를 달성하며 데이터베이스 위치와 네트워크 지연의 관계를 분석하고 최적화했습니다.
복잡한 문제를 단순한 형태로 추상화하고, 변경에 유연한 구조를 설계하는 것을 중요하게 생각합니다.

대량 데이터 처리와 렌더링 성능 최적화 경험을 바탕으로 실시간 데이터 시각화에 기여하고자 합니다.

== 기술 스택
// [수정] JD 관련 기술 우선 배치: React, TypeScript, TanStack Query, TanStack Virtual, Zustand, Vite

React, TypeScript, TanStack Query, TanStack Virtual, Zustand, Vite, Express, PostgreSQL, Prisma
...
==== 주요 성과
// [수정] 그리드 입력 89% 향상, 가상 스크롤 경험 상세 기술

- 그리드 테이블 렌더링 최적화로 입력 반응 시간 89% 향상 (20.38ms → 2.28ms)
    - 학생별 진도/오답 현황을 표시하는 대량 셀(수백 개) 그리드에서 렌더링 지연 발생
    - TanStack Virtual 도입: 뷰포트 내 행만 렌더링하는 가상 스크롤로 DOM 노드 수 최소화
    - 스크롤 시에도 부드러운 60fps 유지, 사용자 입력에 즉각 반응하는 UX 구현
