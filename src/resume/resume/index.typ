...
== 학원 시스템 개발자
입시 학원 강사로 재직하는 동안 수업 품질을 일관되게 유지하기 위해, 구글 스프레드
시트에 학생 오답을 기록하고 파이썬의 pandas로 추출, 조작하였습니다. 이후 더 많은 사람이 이 시스템의 혜택을 보게 하고자 해당 시스템을 웹앱으로 만들었습니다. 이제 더 많은 사람들을 도울 준비가 되었습니다.
...
=== 개인 블로그: terminal to blog << 여기에 아예 내가 만든 타자 연습을 넣을까?? 그것도 괜찮다
...
=== 디자인 라이브러리 iua-react-ui <<<< 여기서는 성과만 적는다
...
- React, Bun, Tanstack Router, Railway, Zustand, Motion, TailwindCSS
...
- 사용자가 컴포넌트 커스터마이제이션을 모든 계층에서 할 수 있도록 node_module에 설치하는 게 아니라 프로젝트 src 폴더에
  파일을 생성하는 방식으로 설치
- bun 번들러와 TanStack Router를 사용하는 도중, TanStack Router의 라우트 생성이 안 되는 문제 발견, 수정 후 PR, 병합됨
