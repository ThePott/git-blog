...
[x] auth me 할 때 토큰 검증해보려고 하는데 500 에러가 뜸. 이유는 아직 모름 <-- 내 토큰이 아닌 카카오 토큰이 저장되어 있어서 그랬음
[x] refresh 로직 제작
[x] 로그아웃 스토어에 제작해야 함
[x] refresh 실패하면 logout 처리
[ ] 관련 내용 블로그에 정리
[ ] refresh 실패하면 reload current route <- 어떻게? -> 상태 바뀌니 자동 리렌더, 가드 될 거 같은데
