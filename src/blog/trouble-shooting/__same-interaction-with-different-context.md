...
#### resonse.data가 다르게 생긴 게 문제이니 api에서 보낼 때 애초에 정리해서 보내자

##### response.data 생김새

- `review/check`: `ExtendedBook`(object)
- `review/check/assignment`: `AssignmentWithBooks[]`(array)

##### 실패

- client에서 사용할 때 이걸 title, subtitle, checkbox로 만드는 거지, title이니 subtitle이니 하는 건 api와 무관하다
